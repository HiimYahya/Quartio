...
const ALERT_STYLES = {
  incident: { bg: 'bg-red-500',    label: 'Incident urgent' },
  contrat:  { bg: 'bg-amber-500',  label: 'Signature requise' },
  vote:     { bg: 'bg-indigo-500', label: 'Nouveau vote' },
  evenement:{ bg: 'bg-green-600',  label: 'Nouvel événement' },
}
...
export default function AlertBanner() {
  ...
  return (
    <div className="space-y-1 px-4 pt-2">
      {alerts.map((alert) => {
        const style = ALERT_STYLES[alert.type] ?? { bg: 'bg-gray-600', label: 'Alerte' }
        return (
          <div key={alert.at}
